...
function addNeglectBadge($info) {
    ...
    const $existing = $actions.find(".cs-neglect-badge");
    ...
    if ($existing.length > 0) {
        // 已经存在就只更新记录的天数（用jQuery data，不碰DOM属性），不删除重建，避免死循环
        $existing.data("days", daysSince);
        return;
    }

    const badge = $(
        `<div class="menu_button menu_button_icon cs-neglect-badge interactable" role="button" tabindex="0">💔</div>`,
    );
    badge.on("click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        alert(`已经 ${$(this).data("days")} 天没有和这个角色聊天了`);
    });
    badge.data("days", daysSince);
...
// 冷落图标点击，弹出天数（捕获阶段拦截，避免触发酒馆自己“点这行打开聊天”）
document.addEventListener(
    "click",
    function (e) {
        const badge = e.target.closest && e.target.closest(".cs-neglect-badge");
        if (!badge) return;
        ...
        const days = $(badge).data("days");
        alert(`已经 ${days} 天没有和这个角色聊天了`);
    },
    true,
);
